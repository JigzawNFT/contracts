...
contract ScriptBase is Script {
  ...
  function _getJigzawNftConfig(Config memory cfg) internal pure returns (JigzawNFT.Config memory) {
    return JigzawNFT.Config({
      owner: cfg.owner,
      minter: cfg.minter,
      devRoyaltyReceiver: cfg.devRoyaltyReceiver,
      devRoyaltyFeeBips: 500, /* 500 bips = 5% */
      defaultImage: DEFAULT_JIGZAW_IMG,
  ...
  function _getMintSwapPoolConfig(Config memory cfg, address jigzawNftAddress) internal pure returns (MintSwapPool.Config memory) {
    return MintSwapPool.Config({
      owner: cfg.owner,
      nft: jigzawNftAddress,
      curve: PoolCurve({
        mintStartId: 801,
        mintEndId: 7056,
  ...
  function _getScriptConfig() internal view returns (Config memory) {
    bool _isLocal = vm.envBool("LOCAL");
