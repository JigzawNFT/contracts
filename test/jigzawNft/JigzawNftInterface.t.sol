...
contract JigzawNftInterface is JigzawNftTestBase {
  function test_SupportsInterfaces() public {
    // Test that the contract supports the ERC165 interface
    assertEq(jigzawNft.supportsInterface(type(IERC165).interfaceId), true, "erc165");
    // Test that the contract supports the ERC721 interface
    assertEq(jigzawNft.supportsInterface(type(IERC721).interfaceId), true, "erc721");
    // Test that the contract supports the ERC721Metadata interface
    assertEq(jigzawNft.supportsInterface(type(IERC721Metadata).interfaceId), true, "erc721metadata");
    // Test that the contract supports the ERC721Enumerable interface
    assertEq(jigzawNft.supportsInterface(type(IERC721Enumerable).interfaceId), true, "erc721enumerable");
    // Test that the contract supports the ERC721Royalty interface
    assertEq(jigzawNft.supportsInterface(type(IERC2981).interfaceId), true, "erc2981");
    // Test that the contract supports the ERC721Royalty interface
    assertEq(jigzawNft.supportsInterface(type(IERC4906).interfaceId), true, "erc4906");
  }
}